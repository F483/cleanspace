...
typedef struct BUP
{
  Uint16 src_len;	//!< source length (bytes)	
  Uint8 src_bpp;		//!< source bitdepth (1,2,4,8)
  Uint8 dst_bpp;		//!< destination bitdepth (1,2,4,8,16,32)
  Uint32 dst_ofs;	//!< {0-30}: added offset {31}: zero-data offset flag
} BUP;
...
typedef	struct
{
	...
	Uint8	*masterp;
	Uint8	*reserved3[3];
	Uint32	system_work2[4];
	Uint8	sendflag;
	Uint8	probe_target_bit;
	Uint8	check_wait;
	Uint8	server_type;
} MultiBootParam;
...
//\{
void SoftReset(void);
void RegisterRamReset(Uint32 flags);
//\}
...
void Halt(void);
void Stop(void);
void IntrWait(Uint32 flagClear, Uint32 irq);
...
//! \name Math functions
//\{
Sint32 Div(Sint32 num, Sint32 den);
Sint32 DivArm(Sint32 den, Sint32 num);
Uint32 Sqrt(Uint32 num);
Sint16 ArcTan(Sint16 dydx);
Sint16 ArcTan2(Sint16 x, Sint16 y);
//\}
...
void CpuSet(const void *src, void *dst, Uint32 mode);
void CpuFastSet(const void *src, void *dst, Uint32 mode);
//\}

Uint32 BiosCheckSum(void);
...
// FIXME void ObjAffineSet(const ObjAffineSource *src, void *dst, Sint32 num, Sint32 offset);
// FIXME void BgAffineSet(const BgAffineSource *src, BgAffineDest *dst, Sint32 num);
//\}
...
//\{
int MultiBoot(MultiBootParam* mb, Uint32 mode);
//\}
...
void VBlankIntrDelay(Uint32 count);
int DivSafe(int num, int den);
int Mod(int num, int den);
Uint32 DivAbs(int num, int den);
int DivArmMod(int den, int num);
Uint32 DivArmAbs(int den, int num);
void CpuFastFill(Uint32 wd, void *dst, Uint32 count);
...
void RegisterRamReset(Uint32 flags);		// swi 01h

// --- Halt ---

void Halt(void);						// swi 02h
void Stop(void);						// swi 03h
void IntrWait(Uint32 flagClear, Uint32 irq);	// swi 04h
